...
from apparun.expressions import ImpactModelExprSet
                ...
                line_errors=[
                    {
                        "loc": ("values",),
                        "msg": "",
                        "type": PydanticCustomError("lists_size_match", ""),
                    }
                ],
            )
            ...
            exprs_sets.append(
                ImpactModelExprSet.build(
                    {name: value[idx] for name, value in list_values.items()},
                    {
                        ...
                        **{
                            param.name: {"type": "enum", "options": list(param.options)}
                            for param in parameters
                            if param.type == "enum"
                        },
                    },
            ...
            for idx, elem in enumerate(value):
                ...
                if invalid_value:
                    errors[(name, elem)] = {
                        "loc": ("values",),
                        "msg": "",
                        "input": exprs_sets[idx][name].expr_raw
                        if exprs_sets[idx][name].complex
                        else None,
                        "type": PydanticCustomError(
                            "value_error",
                            "",

...
applied to impact_model.get_scores() or impact_model.get_nodes_scores() LCIAScores
outputs.
"""
#Equivalent to impact_model.get_scores() outputs with 16 PEF impact categories
score_init = LCIAScores()
score_init.scores = {'EFV3_IONISING_RADIATION': [0.6068699279546468], 
...
# Apply normalisation with 'pef30' method -> using PEF3.0 available normalisation factors ('pef31' also available)
score = score_init.to_normalisation(method='pef30')
# Apply weighting with weighting factors from 'apparun/resources/pef30/weighting_factor.csv' 
...
unique_score = score_init.to_unique_score(isNorm=True, isWeight=True, method='pef30', fileweight='apparun/resources/pef30/weighting_factor.csv')
...
# different model parameters) and 5 PEF impact categories
node_scores = [NodeScores(  name='manufacturing', 
                            parent='ic', 
                            properties=NodeProperties(properties={}), 
                            lcia_scores=LCIAScores(scores={'EFV3_ACIDIFICATION': [0.0029179557018807847, 0.0024434297187895184], 
                                          'EFV3_CLIMATE_CHANGE': [0.46375008402306434, 0.3774843249670958], 
                                          'EFV3_PARTICULATE_MATTER_FORMATION': [1.533898093101055e-08, 1.2721011456798127e-08], 
                                          'EFV3_IONISING_RADIATION': [0.01258464933562949, 0.011859500833723233], 
                                          'EFV3_WATER_USE': [0.18791590390291063, 0.1716230891244127]})), 
                NodeScores( name='use', 
                            parent='ic', 
                            properties=NodeProperties(properties={}), 
                            lcia_scores=LCIAScores(scores={'EFV3_ACIDIFICATION': [0.0007638828481339788, 0.0007638828481339788],
                                          'EFV3_CLIMATE_CHANGE': [0.10350215999388669, 0.10350215999388669],
                                          'EFV3_PARTICULATE_MATTER_FORMATION': [6.57985314092757e-09, 6.57985314092757e-09], 
                                          'EFV3_IONISING_RADIATION': [0.5950072294209235, 0.5950072294209235], 
                                          'EFV3_WATER_USE': [0.16744676484997245, 0.16744676484997245]})), 
                NodeScores( name='eol', 
                            parent='ic', 
                            properties=NodeProperties(properties={}), 
                            lcia_scores=LCIAScores(scores={'EFV3_ACIDIFICATION': [4.7871999999999994e-08, 4.7871999999999994e-08], 
                ...
                NodeScores( name='ic', 
                            parent='', 
                            properties=NodeProperties(properties={}), 
                            lcia_scores=LCIAScores(scores={'EFV3_ACIDIFICATION': [0.0036818864220147635, 0.003207360438923497], 
                                          'EFV3_CLIMATE_CHANGE': [0.567260172816951, 0.48099441376098245], 
                                          'EFV3_PARTICULATE_MATTER_FORMATION': [2.1919067821938125e-08, 1.93010983477257e-08], 
...
unique_node_scores = [score_model_node.to_unique_score(isNorm=True, isWeight=True, method='pef30') for score_model_node in node_scores]

# Convert NodeScores in DataFrame for better visualisation
unique_scores = [score_model_node.to_unpivoted_df() for score_model_node in unique_node_scores]

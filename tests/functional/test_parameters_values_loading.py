...
def test_invalid_values(impact_model):
    """
    Check an exception is raised when the values are
    invalid for their target parameter.
    """
    parameters = {
        "cuda_core": [15.0, -200],
        "lifespan": 4.0,
        "energy_per_inference": "lifespan * 2000",
    }

    expected = [
        {"target_parameter": "cuda_core", "expr": None, "value": 15.0},
        {"target_parameter": "cuda_core", "expr": None, "value": -200},
        {"target_parameter": "lifespan", "expr": None, "value": 4.0},
        {
            "target_parameter": "energy_per_inference",
            "expr": "lifespan * 2000",
            "value": 8000.0,
        },
    ]
    ...
    for err, expect in zip(errors, expected):
        assert err["type"] == "value_error"
        assert err["ctx"]["target_parameter"] == expect["target_parameter"]
        assert err["ctx"]["value"] == expect["value"]
        assert err["input"] == expect["expr"]
